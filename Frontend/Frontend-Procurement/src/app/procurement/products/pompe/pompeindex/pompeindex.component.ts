...
import { PompedialogComponent } from '../pompedialog/pompedialog.component';

@Component({
  selector: 'app-pompeindex',
  templateUrl: './pompeindex.component.html',
  styleUrls: ['./pompeindex.component.css']
})
export class PompeindexComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.AdminUser === 'admin') {
      this.displayedColumns = ['CodePOMPE', 'FournisseurPOMPE', 'MarquePOMPE','TypePOMPE','DebitPOMPE','HauteurPOMPE','TypePrixPOMPE','PrixPOMPE','DetailsPOMPE','HistoriquePOMPE','AddPrixPOMPE']
    } else if (this.AdminUser === 'procurement'){
      this.displayedColumns = ['CodePOMPE', 'FournisseurPOMPE', 'MarquePOMPE','TypePOMPE','DebitPOMPE','HauteurPOMPE','PrixPOMPE','DetailsPOMPE']
    }
    this.allproducts();
  }
  ...
  public allproducts(){
    ...
    return this.http.get(this.url.urlAddress + ":8082/procurement/products/index/POMPE",{headers,responseType:'json' as 'json'})
    .subscribe((data : any)=>{
      ...
      this.dataSource = new MatTableDataSource<any>(this.response);
        ...
        if(data.typeproductprice){
          ...
          data.parametres.typepompe.toLocaleLowerCase().includes(filter)||
          data.parametres.hauteurtotale.toString().includes(filter) ||
            data.parametres.debiteau.toString().includes(filter)
        } else {
            return data.fournisseur.nomf.toLocaleLowerCase().includes(filter)||
            data.marque.toLocaleLowerCase().includes(filter) ||
            data.codep.toLocaleLowerCase().includes(filter) ||
     ...
     switch (sortHeaderId) {
      ...
      case 'FournisseurPOMPE': {
        return data.fournisseur.nomf;
        }
      case 'TypePOMPE': {
        return data.parametres.typepompe;
        }  
      case 'HauteurPOMPE' : {
        return data.parametres.hauteurtotale;
      }
      case 'DebitPOMPE' : {
        return data.parametres.debiteau;
      }
      case 'PrixPOMPE': {
  ...
  openDialog(data:any) {
    ...
    const dialogRef = this.dialog.open(PompedialogComponent,{
      width: '70%',
      height: 'auto',
      data: {
        result : data,
