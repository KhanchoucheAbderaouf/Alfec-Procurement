...
import { ChddialogComponent } from '../chddialog/chddialog.component';

@Component({
  selector: 'app-chdindex',
  templateUrl: './chdindex.component.html',
  styleUrls: ['./chdindex.component.css']
})
export class ChdindexComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.AdminUser === 'admin') {
      this.displayedColumns = ['CodeCHD', 'FournisseurCHD', 'MarqueCHD','TypeCHD','FonctionnementCHD','PuissanceFCHD','TypePrixCHD','PrixCHD','DetailsCHD','HistoriqueCHD','AddPrixCHD']
    } else if (this.AdminUser === 'procurement'){
      this.displayedColumns = ['CodeCHD', 'FournisseurCHD', 'MarqueCHD','TypeCHD','FonctionnementCHD','PuissanceFCHD','PrixCHD','DetailsCHD']
    }
    this.allproducts();
  }
    ...
    .subscribe((data : any)=>{
      ...
      this.dataSource = new MatTableDataSource<any>(this.response);
        ...
        if(data.typeproductprice){
          ...
          data.parametres.typechd.toLocaleLowerCase().includes(filter)||
          data.parametres.fonctionnementchd.toLocaleLowerCase().includes(filter) ||
          data.parametres.puissancecalorifiquechd.toString().includes(filter);
        } else {
            return data.fournisseur.nomf.toLocaleLowerCase().includes(filter)||
            data.marque.toLocaleLowerCase().includes(filter) ||
            data.codep.toLocaleLowerCase().includes(filter) ||
     ...
     switch (sortHeaderId) {
      ...
      case 'TypeCHD': {
        return data.parametres.typechd;
        }  
      case 'PuissanceFCHD' : {
        return data.parametres.puissancecalorifiquechd;
      }
      case 'FonctionnementCHD' : {
        return data.parametres.fonctionnementchd;
      }
      case 'PrixCHD': {
  ...
  openDialog(data:any) {
    ...
    const dialogRef = this.dialog.open(ChddialogComponent,{
      width: '70%',
      height: 'auto',
      data: {
        result : data,
